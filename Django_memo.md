...
- **役割**: アカウント管理に関するURL（登録、ログイン、ログアウト）と処理（ビュー）を紐付ける。
...
データベースの設計図。
- **役割**: タスクとカテゴリのデータをデータベースで管理するための構造（テーブル）定義。
...
- **役割**: タスクを「新しく作る」ときと「内容を書き換える」ときで共通して使う「入力画面」。
